...
describe("focused MVP lightweight UI states", () => {
  ...
  it("21. keeps context selection automatic and exposes chat actions", async () => { const page = await source("components/knowledge/ProjectAssistantPanel.tsx"); assert.match(page, /可用上下文/); assert.match(page, /生成需求概览/); assert.match(page, /总结项目现状/); assert.match(page, /列出待确认事项/); assert.doesNotMatch(page, /<Switch/); });
