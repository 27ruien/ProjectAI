...
describe("focused MVP lightweight UI states", () => {
  ...
  it("11a. keeps failed history out of the default empty state", async () => { const page = await source("components/project/RequirementDocumentsPage.tsx"); assert.match(page, /rows\.find\(\(item\) => item\.status !== "failed"\)/); assert.match(page, /这里不会因你尚未生成而显示错误/); });
  ...
  it("16. renders AI citations with project and template source labels", async () => { const page = await source("components/knowledge/ProjectAssistantPanel.tsx"); assert.match(page, /data-testid="assistant-citations"/); assert.match(page, /\[项目资料\]/); assert.match(page, /\[常规模板\]/); });
  it("17. renders a human AI provider failure", async () => { assert.match(await source("components/knowledge/ProjectAssistantPanel.tsx"), /AI 服务当前不可用。项目资料和常规模板未发生变化，请联系管理员检查模型访问权限。/); });
  it("17a. only opens the guided overview after an explicit user action", async () => { const page = await source("components/knowledge/ProjectAssistantPanel.tsx"); assert.match(page, /showRequirementOverview/); assert.match(page, /生成需求概览/); assert.doesNotMatch(page, /requirement-documents/); });
  ...
  it("21. keeps source selection automatic and exposes chat actions", async () => { const page = await source("components/knowledge/ProjectAssistantPanel.tsx"); assert.match(page, /自动使用/); assert.match(page, /生成需求概览/); assert.match(page, /总结项目现状/); assert.match(page, /列出待确认事项/); assert.doesNotMatch(page, /<Switch/); });
  ...
  it("26. keeps AI provider settings inside the administrator-only settings surface", async () => { const [page, route] = await Promise.all([source("components/system/AiModelManagementPage.tsx"), source("app/[...slug]/page.tsx")]); assert.match(page, /浏览器不会显示、提交或保存 API Key/); assert.match(page, /vectorDimensions/); assert.match(route, /area !== "ai-models"/); });
  ...
  it("23. sends general chat without a knowledge-source preflight", async () => { const page = await source("components/knowledge/ProjectAssistantPanel.tsx"); assert.match(page, /if \(projectId && selectedSourceIds\.length === 0\)/); assert.match(page, /setOptimisticQuestion\(normalized\)/); assert.match(page, /setQuestion\(""\)/); assert.match(page, /正在思考/); assert.match(page, /不使用知识库资料/); });
