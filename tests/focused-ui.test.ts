...
describe("focused MVP lightweight UI states", () => {
  ...
  it("11a. does not turn an old failed generation into the default empty-page error", async () => { const page = await source("components/project/RequirementDocumentsPage.tsx"); assert.match(page, /rows\.find\(\(item\) => item\.status !== "failed"\)/); assert.match(page, /之前的生成没有完成/); });
  ...
  it("17a. hides stale failed artifacts when opening a session", async () => { const page = await source("components/knowledge/ProjectAssistantPanel.tsx"); assert.match(page, /body\.documents\?\.find\(\(item\) => item\.status !== "failed"\)/); });
  ...
  it("21. keeps source selection automatic and exposes chat actions", async () => { const page = await source("components/knowledge/ProjectAssistantPanel.tsx"); assert.match(page, /自动使用/); assert.match(page, /生成需求文档/); assert.match(page, /总结项目现状/); assert.match(page, /列出待确认事项/); assert.doesNotMatch(page, /<Switch/); });
