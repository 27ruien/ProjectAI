...
async function writeFormalSession(output, sha, image, suffix = "0") {
  const session = withDigest({
    ...
    releaseSessionId: `rs-${suffix.repeat(32).slice(0, 32)}`,
    productionBaselineDigest: `sha256:bcfdd818a6560163a04e741d15721f50794c6a43197a963e92716ca45edebb06`,
    createdAt: "2026-01-01T00:00:00Z",
  });
  const filename = path.join(output, "release-session.json");
  await writeFile(filename, JSON.stringify(session));
  return filename;
}

function syntheticReport(type, sha, image, extra = {}) {
  return {
    schemaVersion: 1,
    reportType: type,
    producer: "projectai-release-tool",
    ...
    releaseCandidateSha: sha,
    releaseImageDigest: image,
    releaseSessionId: `rs-${sha.slice(0, 32)}`,
    ...extra,
  };
}
...
async function run(command, args, cwd = root, extraEnv = {}) {
  ...
  if (command !== "session" && !effectiveArgs.some((arg) => arg.startsWith("--session="))) {
    let sha = effectiveArgs.find((arg) => arg.startsWith("--expected-sha="))?.split("=", 2)[1];
    let image = ["rehearse", "restore-drill", "smoke"].includes(command)
      ? effectiveArgs.find((arg) => arg.startsWith("--expected-image="))?.split("=", 2)[1]
      : undefined;
    const inputArg = effectiveArgs.find((arg) => arg.startsWith("--input="));
    let productionBaselineDigest = `sha256:bcfdd818a6560163a04e741d15721f50794c6a43197a963e92716ca45edebb06`;
    if (command === "manifest" && inputArg) {
      const input = JSON.parse(await readFile(inputArg.slice("--input=".length), "utf8"));
      sha = input.releaseCandidateSha;
      image = input.releaseImageDigest;
      productionBaselineDigest = input.productionBaselineDigest;
    }
    ...
    const sessionFile = path.join(
      os.tmpdir(),
      `projectai-release-session-${sha.slice(0, 12)}-${image.slice(7, 19)}.json`,
    );
    await writeFile(sessionFile, JSON.stringify(session));
    effectiveArgs.push(`--session=${sessionFile}`);
  }
  return execFileAsync(process.execPath, [cli, command, ...effectiveArgs], {
    cwd,
    env: { ...process.env, CI: "false", NODE_ENV: "test", ...extraEnv },
  });
}
...
test("manifest is immutable, digest pinned, and contains seven phases", async () => {
  ...
  try {
    ...
    assert.equal(manifest.createdByToolVersion, "b3-c1-v3");
    assert.equal(manifest.producer, "projectai-release-tool");
    assert.equal(manifest.reportType, "release-manifest");
...
test("release session and producer fields are generated by v3 and formal mode rejects JSON fixtures", async () => {
  const output = await temporaryRoot();
  ...
  try {
    await run("session", [
      ...
      `--production-baseline=${productionFixture}`,
      `--output-dir=${output}`,
    ]);
    const sessionFile = path.join(output, "release-session.json");
    const session = JSON.parse(await readFile(sessionFile, "utf8"));
    assert.equal(session.producer, "projectai-release-tool");
    assert.equal(session.producerVersion, "b3-c1-v3");
    assert.equal(session.reportType, "release-session");
    assert.match(session.releaseSessionId, /^rs-[0-9a-f]{32}$/);

    const injected = JSON.parse(await readFile(productionFixture, "utf8"));
    injected.producer = "caller-controlled";
    injected.producerVersion = "wrong";
    injected.reportType = "wrong-type";
    injected.sourceMode = "live-readonly";
    const injectedFile = path.join(output, "injected.json");
    await writeFile(injectedFile, JSON.stringify(injected));
    await run("inventory", [
      "--environment=production",
      `--input=${injectedFile}`,
      `--session=${sessionFile}`,
      `--output-dir=${path.join(output, "inventory")}`,
    ]);
    const inventory = JSON.parse(await readFile(
      path.join(output, "inventory/production-inventory.json"),
      "utf8",
    ));
    assert.equal(inventory.producer, "projectai-release-tool");
    assert.equal(inventory.producerVersion, "b3-c1-v3");
    assert.equal(inventory.reportType, "production-inventory");
    ...
    await assert.rejects(
      ...
      /JSON input is synthetic and allowed only in NODE_ENV=test/,
    );
  } finally {
    await rm(output, { recursive: true, force: true });
  }
...
test("preflight fails closed for held and unknown PostgreSQL migration advisory locks", async () => {
  ...
  try {
    ...
    const fileLockedInventory = withDigest(syntheticReport(
    ...
    for (const advisory of ["held", "unknown"]) {
      ...
      const inventory = withDigest(syntheticReport(
        "production-inventory",
        "2".repeat(40),
        `sha256:${"1".repeat(64)}`,
        inventoryValue,
...
test("Go/No-Go cross-binds reports and rejects checklist, tampering, SHA, image, and missing reports", async () => {
  ...
  try {
    ...
    const rehearsal = await writeArtifactPair({
      ...
      stem: "rehearsal",
      payload: syntheticReport("rehearsal", sha, image, {
    ...
    await writeArtifactPair({
      outputDir: reportRoot,
      stem: "restore",
      payload: syntheticReport("restore-drill", sha, image, {
    ...
    await writeArtifactPair({
      outputDir: reportRoot,
      stem: "smoke",
      payload: syntheticReport("smoke", sha, image, { result: "passed" }),
      markdown: "synthetic smoke report",
    });
    await writeArtifactPair({
      outputDir: reportRoot,
      stem: "rollback",
      payload: syntheticReport("rollback-check", sha, image, {
    ...
    await writeArtifactPair({
      outputDir: reportRoot,
      stem: "disabled",
      payload: syntheticReport("disabled-image", sha, image, { passed: true }),
      markdown: "synthetic disabled-image report",
    });
    await writeArtifactPair({
      outputDir: reportRoot,
      stem: "old-app",
      payload: syntheticReport("old-app-compatibility", sha, image, {
    ...
    const ciDatabase = await writeArtifactPair({
      outputDir: reportRoot,
      stem: "ci-database",
      payload: syntheticReport("database-rehearsal", sha, image, {
        sourceMode: "ci-artifact",
        passed: true,
      }),
      markdown: "CI database rehearsal",
    });
    const ciDisabled = await writeArtifactPair({
      outputDir: reportRoot,
      stem: "ci-disabled",
      payload: syntheticReport("disabled-image", sha, image, {
        sourceMode: "ci-artifact",
        passed: true,
      }),
      markdown: "CI disabled image",
    });
    const ciSmoke = await writeArtifactPair({
      outputDir: reportRoot,
      stem: "ci-smoke",
      payload: syntheticReport("smoke", sha, image, {
        sourceMode: "ci-artifact",
        result: "passed",
      }),
      markdown: "CI smoke",
    });
    const artifactReportDigestMap = [
      ["release-database-rehearsal.json", ciDatabase],
      ["release-disabled-image-rehearsal.json", ciDisabled],
      ["release-smoke.json", ciSmoke],
    ].map(([filename, report]) => ({
      filename,
    ...
    await writeArtifactPair({
      outputDir: reportRoot,
      stem: "ci-evidence",
      payload: syntheticReport("ci-evidence", sha, image, {
        sourceMode: "ci-artifact",
        status: "success",
        headSha: sha,
        headBranch: "agent/production-release-readiness",
        workflowRunId: "123",
        artifactId: "1234",
        artifactName: "product-review-evidence-123-1",
        artifactDigest: `sha256:${"c".repeat(64)}`,
        artifactReportDigestMap,
      }),
    ...
    const goArgs = [
      ...
      `--ci-database-rehearsal=${path.join(reportRoot, "ci-database.json")}`,
      `--ci-disabled-image=${path.join(reportRoot, "ci-disabled.json")}`,
      `--ci-smoke=${path.join(reportRoot, "ci-smoke.json")}`,
    ];
    ...
    const wrongSha = withDigest({
      ...
      digest: undefined,
      result: "passed",
      releaseCandidateSha: "3".repeat(40),
    });
    ...
    const manualPassedFile = path.join(output, "manual-passed.json");
    await writeFile(
      manualPassedFile,
      JSON.stringify(withDigest({ schemaVersion: 1, passed: true, result: "passed" })),
    );
    await assert.rejects(
      run("go-no-go", goArgs.map((arg) =>
        arg.startsWith("--smoke=") ? `--smoke=${manualPassedFile}` : arg)),
      /report type|producer/i,
    );

    for (const [label, changes, pattern] of [
      ["wrong-producer-version", { producerVersion: "b3-c1-v2" }, /producer version/],
      ["wrong-report-type", { reportType: "disabled-image" }, /report type/],
    ]) {
      const original = JSON.parse(await readFile(path.join(reportRoot, "smoke.json"), "utf8"));
      const changed = withDigest({ ...original, digest: undefined, ...changes });
      const filename = path.join(output, `${label}.json`);
      await writeFile(filename, JSON.stringify(changed));
      await assert.rejects(
        run("go-no-go", goArgs.map((arg) =>
          arg.startsWith("--smoke=") ? `--smoke=${filename}` : arg)),
        pattern,
      );
    }

    const differentSessionSmoke = JSON.parse(
      await readFile(path.join(reportRoot, "smoke.json"), "utf8"),
    );
    const differentSessionFile = path.join(output, "different-session-smoke.json");
    await writeFile(differentSessionFile, JSON.stringify(withDigest({
      ...differentSessionSmoke,
      digest: undefined,
      releaseSessionId: `rs-${"9".repeat(32)}`,
    })));
    await assert.rejects(
      run("go-no-go", goArgs.map((arg) =>
        arg.startsWith("--smoke=") ? `--smoke=${differentSessionFile}` : arg),
        root,
        commandAdapter(),
      ),
      (error) => error.code === 2,
    ...
    const ciEvidenceValue = JSON.parse(
      await readFile(path.join(reportRoot, "ci-evidence.json"), "utf8"),
    );
    for (const [label, mutate] of [
      ["missing-ci-map-entry", (value) => {
        value.artifactReportDigestMap = value.artifactReportDigestMap.slice(1);
      }],
      ["mismatched-ci-report-digest", (value) => {
        value.artifactReportDigestMap[0].reportDigest = `sha256:${"e".repeat(64)}`;
      }],
      ["artifact-name-run-mismatch", (value) => {
        value.artifactName = "product-review-evidence-999-1";
      }],
      ["artifact-head-mismatch", (value) => {
        value.headSha = "3".repeat(40);
      }],
    ]) {
      const changed = structuredClone(ciEvidenceValue);
      mutate(changed);
      const filename = path.join(output, `${label}.json`);
      await writeFile(filename, JSON.stringify(withDigest({ ...changed, digest: undefined })));
      await assert.rejects(
        run("go-no-go", goArgs.map((arg) =>
          arg.startsWith("--ci-evidence=") ? `--ci-evidence=${filename}` : arg)),
        (error) => error.code === 2,
      );
    }
  } finally {
...
test("disabled-image TSV becomes digest-pinned sanitized rehearsal evidence", async () => {
  ...
  try {
    const session = await writeFormalSession(output, sha, image, "1");
    await writeFile(
      input,
      [
...
test("old-app evidence records shell-only rollback scope and observed database absence", async () => {
  ...
  try {
    const session = await writeFormalSession(
      output,
      sha,
      `sha256:${"2".repeat(64)}`,
      "2",
...
test("route status contract rejects 404, 401, 403, and 500 instead of treating them as smoke success", async () => {
  const output = await temporaryRoot();
  const sha = "1".repeat(40);
  const candidateImage = `sha256:${"2".repeat(64)}`;
  const rollbackImage = `sha256:${"a".repeat(64)}`;
  try {
    const session = await writeFormalSession(output, sha, candidateImage, "3");
    async function runDisabled(login, projects, label) {
      const input = path.join(output, `disabled-${label}.tsv`);
      await writeFile(input, [
        `appImageDigest\t${candidateImage}`,
        `dbToolsImageDigest\tsha256:${"3".repeat(64)}`,
        `expectedSha\t${sha}`,
        "health\thealthy",
        "assistantEnabled\tfalse",
      ...
      return execFileAsync(process.execPath, [
        ...
        `--input=${input}`,
        `--expected-sha=${sha}`,
        `--expected-image=${candidateImage}`,
        `--session=${session}`,
        `--output-dir=${path.join(output, label)}`,
      ], { cwd: root });
    }
    async function runOld(login, dashboard, projects, label) {
      const input = path.join(output, `old-${label}.tsv`);
      await writeFile(input, [
        `productionImageDigest\t${rollbackImage}`,
        `dbToolsImageDigest\tsha256:${"3".repeat(64)}`,
        "targetMigration\t7",
        "migrationCount\t8",
        "pgvectorVersion\t0.8.1",
        `oldAppLoginStatus\t${login}`,
        `oldAppDashboardStatus\t${dashboard}`,
        `oldAppProjectsStatus\t${projects}`,
        "oldAppRestartCount\t0",
        "databaseUrlSuppliedToOldApp\ttrue",
        "oldAppOperationalWithParallel0007Database\ttrue",
        "oldAppDatabaseDependency\tabsent",
      ...
      return execFileAsync(process.execPath, [
        ...
        `--input=${input}`,
        `--expected-sha=${sha}`,
        `--expected-image=${rollbackImage}`,
        `--session=${session}`,
        `--output-dir=${path.join(output, label)}`,
      ], { cwd: root });
    }

    await runDisabled(200, 307, "allowed-disabled");
    await runOld(200, 302, 307, "allowed-old");
    await assert.rejects(runDisabled(404, 307, "login-404"));
    await assert.rejects(runOld(200, 404, 307, "dashboard-404"));
    await assert.rejects(runOld(200, 302, 404, "projects-404"));
    for (const status of [401, 403, 500]) {
      await assert.rejects(runOld(200, status, 307, `status-${status}`));
    }
  } finally {
    await rm(output, { recursive: true, force: true });
  }
