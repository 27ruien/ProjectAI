...
export async function POST(request: Request) { try {
  ...
  if (input.action === "set_generation_model_enabled") {
    const [model] = await db.select().from(aiGenerationModel).where(and(eq(aiGenerationModel.id, input.modelId), eq(aiGenerationModel.organizationId, input.organizationId))).limit(1);
    if (!model) throw new ProjectAssistantError(404, "AI_MODEL_PROFILE_NOT_FOUND", "模型不存在");
    if (input.enabled && model.lastTestStatus !== "passed") throw new ProjectAssistantError(409, "AI_MODEL_PROFILE_DISABLED", "文本模型需先通过 JSON 能力测试后才能启用");
    await db.update(aiGenerationModel).set({ enabled: input.enabled, updatedBy: principal.user.id, updatedAt: new Date() }).where(eq(aiGenerationModel.id, model.id));
  }
