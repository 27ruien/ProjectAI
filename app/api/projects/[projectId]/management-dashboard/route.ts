...
import { projectDashboard } from "@/lib/project-management/work-management";
export async function GET(request: Request, context: { params: Promise<{ projectId: string }> }) { try { const { projectId } = await context.params; const principal = await requireApiPrincipal(request.headers); return jsonResponse(await projectDashboard({ principal, projectId, requestHeaders: request.headers })); } catch (error) { return projectManagementErrorResponse(error); } }
