...
sudo awk -F= '
  BEGIN { auth=0; mock=0; staging_login=0; rate=0; org=0; daily=0; sync=0 }
  ...
  $1 == "ALLOW_DEBUG_IDENTITY" { next }
  $1 == "ALLOW_STAGING_TEST_LOGIN" { print "ALLOW_STAGING_TEST_LOGIN=true"; staging_login += 1; next }
  ...
  END {
    ...
    if (!staging_login) print "ALLOW_STAGING_TEST_LOGIN=true"
