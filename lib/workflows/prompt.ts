...
eventId/key 必须稳定 snake_case；eventType 只能是 page_view、click、select、permission、ai、result、error、custom；valueType 只能是 string、number、boolean、date、array、TBD，无法从 evidence 确认时必须写 TBD；coreEvent 只能是 JSON boolean；Coverage status 只能是 covered、gap、pending。events[] 每行只描述一个事件参数；同一事件有多个参数时可以重复 eventId，但 eventName/coreEvent/eventType/description 必须一致且 parameterKey 必须不同。每条 requirementEventCoverage/pageEventMatrix 的 requirement/page/eventId 都必须是单个字符串；status=covered 时 eventId 必须逐字复制当前 events[] 中某一项的 eventId；没有合同映射时 status 必须保留 gap/pending 且 eventId 必须写 TBD。需求、页面或事件存在多对多关系时拆成多行，禁止数组、对象、事件名或不存在的 ID。若 evidence 明确给出事件或参数合同，eventName、eventId、parameterKey 和 publicParameters.key 必须逐字复制来源中的 snake_case 标识，禁止改名、同义改写或新增未在合同中出现的分析事件。未知 Measurement ID 写 TBD。`;
  }
