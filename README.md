...
- 项目空间：概览、真实资料、真实项目知识搜索，以及仍为 Mock 的需求、Scope、Action、会议和风险模块。
- 项目知识：读取当前项目 Active/Current/Succeeded 索引，支持 FTS、contains、`pg_trgm` 模糊匹配与 PDF Page、DOCX Section、XLSX Range、PPTX Slide、文本行来源；不生成 AI 答案。
...
项目知识页当前是真实词法搜索与来源定位，所有查询绑定 `projectId` 并按当前有效版本过滤。真实文件仍不会交给 AI；当前没有 OCR、Embedding、RAG、Reranker、Qwen 或真实模型调用。
...
- 仍为 Mock：需求、Scope、Action、会议、风险、审核和 AI execution。
- 未实现：OCR、Embedding、pgvector、Hybrid Search、RAG、真实模型和 Provider Key。
...
Playwright report、test results、trace/video 和运行时上传原件只保留在 CI 工作区，不进入产品 Evidence。发布 Payload 采用强 allowlist，只包含索引/脱敏报告、22 张约定 PNG 和固定名称的 UTF-8 日志，并扫描 Session、MinIO/S3 凭据、Bucket/Endpoint/Object Key 与编码变体；Manifest 读取每张 PNG 的实际尺寸。完整策略见 [docs/TESTING.md](./docs/TESTING.md)。
...
v0.5 B2 只允许部署 Staging，禁止在 Production 服务器构建、重启、迁移、增加 Worker/对象存储或重新部署本应用。现有 Production 继续使用 `/tool/projectai`、`/srv/projectai`、`project-ai-os` 和 `127.0.0.1:3100`；发布前后必须精确比对 Production 容器身份、运行状态、restart count 与 health。
