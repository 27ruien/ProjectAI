...
- 向量基础：固定 `qwen-text-embedding-cn-v1` Profile、`text-embedding-v4`、1024 维 pgvector、Chunk Embedding、持久化 Job/Batch、专用 Worker、Lease/Retry/Recovery、增量生成、dry-run Backfill、Probe、Usage 与每日上限；不接入浏览器检索或回答 Evidence。
...
项目知识页继续保留真实词法搜索与来源定位，并由服务端内部检索为项目助手提供最多 10 条、总计最多 24000 字符的 Evidence。没有合格 Evidence 时不调用模型；有 Evidence 时由 Qwen 生成回答，服务端只接受本次 Evidence 标记并生成公开来源 DTO。当前已有受保护的 Embedding/pgvector 基础，但搜索与助手 Evidence 没有接入向量；OCR、Hybrid Retrieval、RRF、ANN 索引和 Reranker 仍未实现。
...
- 真实 PostgreSQL AI 状态：模型 Profile、私人 Thread、Message、Execution、Citation、Token Usage、限流和 Audit；Embedding Profile/Job/Batch/Chunk Vector 只供 Worker 与受保护运维使用。
- 仍为 Mock：需求、Scope、Action、会议、风险、审核任务和相关生成工作流。
- 未实现：OCR、用户向量检索、Hybrid Retrieval、RRF、Rerank、Tool Calling 和正式业务写入。
