...
export function Workspace({
  ...
  const isProjectDetail =
    ...
    entityId !== "new";
  const project =
    isProjectDetail && currentProject?.id === entityId
      ? currentProject
      : undefined;
  let page: React.ReactNode;
  if (section === "assistant") page = <FocusedChatPage viewer={viewer} />;
  else if (isDataSpace && area === "projects" && !entityId)
    page = <ProjectsPage viewer={viewer} />;
  else if (isDataSpace && area === "projects" && entityId === "new")
    page = viewer.canCreateProject ? (
      <CreateProjectPage managerName={viewer.user.displayName} />
    ) : (
      <AccessDeniedPage />
    );
  else if (isProjectDetail && !project)
    ...
    page = <CompanyKnowledgePage />;
  else if (
    section === "organization" &&
    viewer.user.productRole === "super_admin"
  )
    page = (
      <StandardPage>
        <OrganizationPage mode={area === "members" ? "members" : "structure"} />
      </StandardPage>
    );
  else if (section === "admin" && area === "models")
    page = (
      <StandardPage>
        <AiModelManagementPage
          organizationId={viewer.aiConfigurationOrganizationId ?? ""}
          verificationProjectId={viewer.projects[0]?.id ?? null}
        />
      </StandardPage>
    );
  ...
  else if (section === "help" && area === "models-and-api")
    page = <ModelsAndApiHelpPage />;
  else
    page = (
      <StandardPage>
