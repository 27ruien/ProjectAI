...
import { FilterBar, PageHeader, PageShell } from "@/components/common";
...
export function ProjectsPage({ viewer }: { viewer: ViewerContext }) {
  ...
  return <PageShell className="min-h-full" data-testid="projects-page">
    <PageHeader eyebrow="资料空间" title="项目资料" description="每个项目拥有独立的资料、成员权限与 AI 产物。" actions={viewer.canCreateProject ? <CreateProjectDialog managerName={viewer.user.displayName} trigger={<Button><Plus />创建项目</Button>} /> : null} />
    ...
    <Card className="overflow-hidden p-0"><FilterBar value={query} onChange={setQuery} placeholder="搜索项目、部门或负责人"><Select value={status} onValueChange={setStatus}><SelectTrigger className="w-full sm:w-40" aria-label="按状态筛选"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">全部状态</SelectItem><SelectItem value="planning">规划中</SelectItem><SelectItem value="active">进行中</SelectItem><SelectItem value="completed">已完成</SelectItem></SelectContent></Select></FilterBar></Card>
