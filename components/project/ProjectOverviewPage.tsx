...
export function ProjectOverviewPage({ project }: { project: AuthorizedProjectSummary }) {
  ...
  const runLifecycleAction = async () => { const action = confirmAction; if (!action) return; setError(null); const response = await fetch(withBasePath(`/api/projects/${project.id}`), action === "delete" ? { method: "DELETE", credentials: "include" } : { method: "PATCH", credentials: "include", headers: { "content-type": "application/json" }, body: JSON.stringify({ status: "archived" }) }); if (!response.ok) { const body = await response.json() as { error?: { message?: string } }; setError(body.error?.message ?? (action === "delete" ? "删除失败" : "关闭失败")); setConfirmAction(null); return; } setConfirmAction(null); if (action === "delete") router.push("/projects"); router.refresh(); };
  const actions = <>{project.permissions.canEditProject ? <Button variant="outline" onClick={() => setEditing(true)}><Pencil />编辑项目</Button> : null}{project.permissions.canDeleteProject ? <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" aria-label="更多项目操作"><MoreHorizontal /></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onSelect={() => setConfirmAction("close")}>关闭项目</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem variant="destructive" onSelect={() => setConfirmAction("delete")}>删除空项目</DropdownMenuItem></DropdownMenuContent></DropdownMenu> : null}</>;
