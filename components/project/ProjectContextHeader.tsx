...
export function ProjectContextHeader({ project, activeTab, actions }: { project: AuthorizedProjectSummary; activeTab: ProjectTab; actions?: ReactNode }) {
  return <div className="border-b bg-card">
    <div className="px-5 pt-5 sm:px-6 lg:px-8">
      <Breadcrumb className="mb-3"><BreadcrumbList className="text-xs"><BreadcrumbItem><BreadcrumbLink asChild><Link href="/knowledge/projects">项目</Link></BreadcrumbLink></BreadcrumbItem><BreadcrumbSeparator /><BreadcrumbItem><BreadcrumbPage>{project.name}</BreadcrumbPage></BreadcrumbItem></BreadcrumbList></Breadcrumb>
