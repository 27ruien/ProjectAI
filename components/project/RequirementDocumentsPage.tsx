...
export function RequirementDocumentsPage({ project }: { project: AuthorizedProjectSummary }) {
  ...
  useEffect(() => { if (busy !== "generate") return; const timer = window.setInterval(() => setGenerationStep((value) => Math.min(value + 1, steps.length - 1)), 1800); return () => window.clearInterval(timer); }, [busy]);
  ...
  const generate = async () => { setBusy("generate"); setGenerationStep(0); setError(null); try { const id = await request(`/api/projects/${project.id}/requirement-documents`, { method: "POST", headers: { "content-type": "application/json" }, body: "{}" }); await load(id); } catch (caught) { setError(caught instanceof Error ? caught.message : "生成失败"); await load(); } finally { setBusy(null); } };
  ...
  return <div className="min-h-full" data-testid="requirement-documents-page">
    ...
    <div className="px-5 py-7 sm:px-6 lg:px-8">
      <header className="mb-6 flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-lg font-semibold">需求文档</h2><p className="mt-1 text-sm text-muted-foreground">基于当前项目资料和已发布公司规范生成，所有结果均需人工审核。</p></div>{canEdit ? <Button onClick={() => void generate()} disabled={Boolean(busy)}>{busy === "generate" ? <LoaderCircle className="animate-spin" /> : <Sparkles />}基于最新资料生成新版本</Button> : null}</header>
      ...
      {!selected ? <section className="grid min-h-96 place-items-center rounded-lg border bg-card text-center" data-testid="requirement-empty"><div><FileText className="mx-auto size-9 text-muted-foreground" /><h3 className="mt-3 text-sm font-medium">尚无需求文档</h3><p className="mt-1 text-xs text-muted-foreground">上传并解析项目资料后生成第一版。</p>{canEdit ? <Button className="mt-4" onClick={() => void generate()}><Sparkles />生成第一版</Button> : null}</div></section> : selected.status === "failed" ? <RequirementFailure projectId={project.id} document={selected} canEdit={canEdit} busy={Boolean(busy)} onGenerate={() => void generate()} /> : <>
...
function RequirementFailure({ projectId, document, canEdit, busy, onGenerate }: { projectId: string; document: RequirementDocument; canEdit: boolean; busy: boolean; onGenerate: () => void }) {
  const providerFailure = document.failureCode === "REQUIREMENT_PROVIDER_FAILED";
  return <section className="mx-auto max-w-3xl py-12" data-testid="requirement-provider-failure"><Alert variant="destructive" className="border-destructive/20 bg-destructive-soft"><AlertTitle>{providerFailure ? "AI 服务暂时不可用" : "需求文档未生成"}</AlertTitle><AlertDescription className="leading-6">{failureMessages[document.failureCode ?? ""] ?? "需求文档生成未完成，请检查资料状态后手动重试。"}</AlertDescription></Alert><div className="mt-4 flex flex-wrap gap-2">{canEdit ? <Button onClick={onGenerate} disabled={busy}><Sparkles />重新生成</Button> : null}<Button variant="outline" asChild><Link href={`/projects/${projectId}/files`}>返回项目资料</Link></Button></div></section>;
}
