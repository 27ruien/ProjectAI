...
export function RequirementDocumentsPage({ project }: { project: AuthorizedProjectSummary }) {
  ...
  const load = useCallback(async (preferredId?: string) => {
    ...
    const next = rows.find((item) => item.id === preferredId) ?? rows.find((item) => item.status !== "failed") ?? null;
  ...
  return <div className="min-h-full" data-testid="requirement-documents-page">
    ...
    <div className="px-5 py-7 sm:px-6 lg:px-8">
      <header className="mb-6 flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-lg font-semibold">AI 生成文档</h2><p className="mt-1 text-sm text-muted-foreground">这里保存 AI 助手生成并已保存到项目的文档、资料快照和历史版本。</p></div>{canEdit ? <Button asChild><Link href={`/assistant?project=${encodeURIComponent(project.id)}`}><Sparkles />前往 AI 助手</Link></Button> : null}</header>
      ...
      {!selected ? <section className="mt-6 grid min-h-52 place-items-center rounded-lg border bg-card text-center" data-testid="requirement-empty"><div><FileText className="mx-auto size-9 text-muted-foreground" /><h3 className="mt-3 text-sm font-medium">尚无 AI 生成文档</h3><p className="mt-1 text-xs text-muted-foreground">在 AI 助手中发起需求概览或其他专业任务，保存后会出现在这里；尚未生成不是错误。</p>{canEdit ? <Button className="mt-4" asChild><Link href={`/assistant?project=${encodeURIComponent(project.id)}`}><Sparkles />前往 AI 助手</Link></Button> : null}</div></section> : selected.status === "failed" ? <RequirementFailure projectId={project.id} document={selected} /> : <>
...
function RequirementFailure({ projectId, document }: { projectId: string; document: RequirementDocument }) {
  ...
  return <section className="mx-auto max-w-3xl py-12" data-testid="requirement-provider-failure"><Alert variant="destructive" className="border-destructive/20 bg-destructive-soft"><AlertTitle>{providerFailure ? "AI 服务暂时不可用" : "需求文档未生成"}</AlertTitle><AlertDescription className="leading-6">{failureMessages[document.failureCode ?? ""] ?? "需求文档生成未完成，请检查资料状态后在 AI 助手中重试。"}</AlertDescription></Alert><div className="mt-4 flex flex-wrap gap-2"><Button asChild><Link href={`/assistant?project=${encodeURIComponent(projectId)}`}><Sparkles />返回 AI 助手</Link></Button><Button variant="outline" asChild><Link href={`/data-spaces/projects/${projectId}/files`}>查看项目资料</Link></Button></div></section>;
}
