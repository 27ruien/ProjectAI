  ...
  return (
    <div className="space-y-5">
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="flex items-center gap-2 text-sm font-semibold"><FileText className="size-4 text-primary" />选择工作流资料</h2><p className="mt-1 text-xs text-muted-foreground">从知识库选择现有文档，或上传一份 24 小时有效的临时附件。</p></div><button type="button" disabled={["loading", "uploading", "running", "reviewing"].includes(phase)} onClick={() => fileInput.current?.click()} className="inline-flex h-9 items-center gap-2 rounded-lg border px-3 text-xs font-medium disabled:opacity-50">{phase === "uploading" ? <LoaderCircle className="size-4 animate-spin" /> : <Upload className="size-4" />}上传附件</button><input ref={fileInput} type="file" hidden accept=".pdf,.docx,.xlsx,.pptx,.txt,.md" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadTemporary(file); }} /></div>
