    ...
    <div className="space-y-6">
      ...
      {!runId ? (
        <section className="rounded-2xl border border-border bg-card p-5">
          <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="flex items-center gap-2 text-sm font-semibold"><FileText className="size-4 text-primary" />选择授权资料</h2><p className="mt-1 text-xs text-muted-foreground">当前项目、已授权部门/公司来源及 24 小时临时附件。手工选择只能缩小授权范围。</p></div><button type="button" onClick={() => fileInput.current?.click()} className="inline-flex h-9 items-center gap-2 rounded-lg border border-border px-3 text-xs font-medium"><Upload className="size-4" />上传临时附件</button><input ref={fileInput} hidden type="file" accept=".md,.txt,.pdf,.docx,.pptx,.xlsx" onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(file); }} /></div>
