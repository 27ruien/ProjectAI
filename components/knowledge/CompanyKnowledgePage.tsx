...
export function CompanyKnowledgePage() {
  ...
  const load = useCallback(async () => { const response = await fetch(withBasePath("/api/company-knowledge"), { credentials: "include", cache: "no-store" }); const body = await response.json() as { documents?: CompanyDocument[]; canManage?: boolean; error?: { message?: string } }; if (!response.ok) throw new Error(body.error?.message ?? "常规模板加载失败"); setDocuments(body.documents ?? []); setCanManage(Boolean(body.canManage)); }, []);
  useEffect(() => { const timer = window.setTimeout(() => { void load().catch((caught) => setError(caught instanceof Error ? caught.message : "常规模板加载失败")); }, 0); return () => window.clearTimeout(timer); }, [load]);
  useEffect(() => {
  ...
  return <main className="min-h-full px-5 py-7 sm:px-6 lg:px-8" data-testid="company-knowledge-page">
    <header className="mb-6 flex flex-wrap items-end justify-between gap-4"><div><h2 className="text-xl font-semibold tracking-tight">常规模板</h2><p className="mt-1.5 text-sm text-muted-foreground">维护公司规范与标准模板；会话按问题自动使用已发布且有权访问的当前版本。</p></div>{canManage ? <Button onClick={() => setUploadOpen(true)}><Upload />上传模板</Button> : null}</header>
    {error ? <Alert variant="destructive" className="mb-4"><AlertDescription>{error}</AlertDescription></Alert> : null}
    <div className="mb-4 flex flex-wrap gap-2"><label className="relative min-w-56 flex-1 sm:max-w-sm"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索常规模板" className="pl-9" /></label><Select value={category} onValueChange={(value) => setCategory(value as typeof category)}><SelectTrigger className="w-40"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">全部类别</SelectItem>{Object.entries(categoryLabels).map(([value, label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select><Select value={status} onValueChange={(value) => setStatus(value as typeof status)}><SelectTrigger className="w-32"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">全部状态</SelectItem>{Object.entries(lifecycleLabels).map(([value, label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select></div>
