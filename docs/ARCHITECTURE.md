...
B3-C2A 在 Readiness 之上增加独立的 Rollout control plane：固定 Fingerprint Trust Anchor、单 Phase/Action/Marker Authorization、不可变的逐 Authorization 原子 claim、受保护的 Digest-linked Replay Journal、Apply/Verify 分离、Lease Lock、倒序 Rollback Verification、Finalize 和 Manifest Image/CWD contracts。Deployment Lock 以 fsync 后的完整文件做 no-replace 原子发布；Acquire/Update/Release/Clear 共用固定 lifecycle guard 目录，预先 fsync 的完整 metadata 以 hard-link no-replace 取得所有权，只有空目录可安全复用，完整 guard 只可 rename 为永久 `guardId + Digest` receipt，禁止按固定路径 unlink，从而同时关闭 publish-crash wedge 与 stale takeover ABA；非空未知内容或损坏 metadata 直接失败关闭且不得覆盖。每次 CAS 同时绑定 Session、Lock ID、轮换 Lease Token、PID/hostname/UID 与记录 Digest，并在取得 guard、读取 exact record 后重新采样时钟，避免等待时间复活已过期 Lease；长命令期间持续 Heartbeat，stale Lock 或 orphan guard 只能经 review + signed clear 后恢复。Phase/Verify/Rollback/Finalize 会在使用 replacement 前消耗并 reconcile 延迟 clear approval。Phase Journal 用 previous-Digest append claim 串行化并发写入；claim-before-append 崩溃只允许在同主机同 UID、超时且 PID 确认死亡后，通过固定 recovery guard 补写 claim 内唯一事件，等待循环每次重新采样时钟，live/remote/不确定状态保持失败关闭；若恢复的是另一事件，调用方必须因 `PRODUCTION_ROLLOUT_STATE_CHANGED` 中止并重新采集状态。Finalize 不信任单独 Journal state，而是联合验证七份 durable Verification Report、Journal/report Digest、最终 live Inventory、七类 active count 和 App/双 Worker实际 Image，以 `prepared → exact release → completed` 两阶段记录，并只接受显式确认、超时且 dead-owner 的恢复；两个 reacquire 分支若确认 Journal tail 已变化，只能通过全身份 CAS 领取并释放刚创建的 idle Lock，未知 append 状态则保留该 Lock 供 immutable claim 与 Journal reconcile，不能删除已变化或 active Lock。Production claim helper 在加载业务模块前先固定自身、Trust Contract 与完整依赖 bundle 的路径、owner、mode、symlink 状态和 Digest。Image Transfer 必须取得并验证结构化 claim receipt，成功前禁止 `mkdir`、`scp` 与 `docker load`。Production 网络拆为私有数据网络与仅 App/Embedding Worker可加入的 Egress；Runtime 产品页面、ProjectKnowledgeService 与 AIGateway 边界不变。
