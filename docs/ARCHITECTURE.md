...
- 应用健康检查调用 `${basePath}/api/health`；只有 PostgreSQL 认证/查询成功且 `users`、`sessions`、`projects`、`project_members` 四张已提交核心表可查询时才返回 `status: ok`，失败只返回通用 `503`，不泄露计数、Schema 或连接信息。
...
测试生成的 `playwright-report/`、`review-artifacts/`、`test-results/` 和 `test-logs/` 是 CI 工作区内的原始输入，不直接上传。最终步骤复制它们到 `product-review-evidence/`，结合运行时环境 Secret 与必须可查询的数据库 Session Token 做精确值扫描；所有文本执行结构化 Session/Cookie 清理，改名、嵌套或 SFX ZIP 与 gzip 由 magic 识别，归档 entry 路径一并清理，ZIP Data URI 支持 MIME、大小写、空白、Base64 和原始 percent-encoding 变体。Secret 集合同时覆盖 URL、JSON、标准/URL-safe/折行 Base64 与组合编码。归档递归解包、脱敏、复核并重建；无法保证安全的二进制/普通归档即使已删除或替换为 omission 说明，也会让整个步骤失败。数据库 Session 不可核验、内嵌归档无法解析或 manifest 一致性错误同样失败关闭。成功运行必须有 6 张必需截图；失败/取消运行允许 manifest 明确列出缺失截图并上传已安全清洗的日志。最终树与合同复核通过后写入 `sanitization-report.json`，GitHub Actions 只上传这个目录；脱敏步骤失败会使 CI 失败且跳过 artifact 上传。
