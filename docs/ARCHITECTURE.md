...
Database Rehearsal 使用独立虚构非空数据库完成 custom dump、Checksum、Restore 和 0004–0007，不连接 Production。Release Candidate 的 runner/db-tools Image 单独构建，AI 全关闭演练不挂 Qwen Secret、不发布端口、不产生 Job/Query Call/Execution。Evidence sanitizer 只允许固定 Release Report 名称进入 Payload A，并在上传后由 Provenance B 引用真实 Artifact ID/Digest。
