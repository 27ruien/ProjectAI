...
B3-C2A 在 Readiness 之上增加独立的 Rollout control plane：Ed25519 Authorization verifier、固定 Phase 状态机、原子 Lock、Digest-linked append-only Journal、Status/Resume/Rollback、Observation/Cost/Stop Gates 和 Production Compose/Secret/Image contracts。该控制面与 Runtime 请求路径分离；默认没有 formal Authorization，CI 只使用 ephemeral test key、Fake Provider 和隔离 Compose。Runtime 产品页面、ProjectKnowledgeService 与 AIGateway 边界不变。
