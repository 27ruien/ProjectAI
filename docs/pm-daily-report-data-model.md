...
基础 Migration 为 `drizzle/0016_tricky_revanche.sql`。追加 Migration `drizzle/0017_nosy_boomer.sql` 为任务增加加班工时、紧急重要度快照与进度，并把正常工时约束升级为非负 0.25 小时精度、正常与加班合计不超过 24 小时。0017 不覆盖历史 Migration，旧任务的新列保持 `null`。
...
| `timesheet_tasks` | 草稿中的结构化任务 | 正常/加班工时、可空紧急度/进度、来源记录 ID、人工审核状态 |
...
本地/CI 的升级测试会创建隔离临时数据库，先应用 0000–0015、插入非空旧数据，再应用 0016、插入历史日报任务，最后应用 0017；核对旧数据、新列、四项工时/进度约束、触发器和活动批次索引，最后只删除该临时数据库。
