...
- Migration `drizzle/0004_groovy_nightcrawler.sql` 新增 pgvector Extension、Profile、Embedding Job/Batch/Vector 表、跨项目复合约束和 Chunk 失效触发器；本轮只新增 `drizzle/0005_durable_embedding_calls.sql`，不修改已部署的 `0004`，并补齐 durable Provider 状态、预算预留与 Worker heartbeat。
...
- 启用顺序固定为：PostgreSQL/MinIO 备份 → 新代码 Flag=false 在旧 Schema 健康 → `0005` Migration/pgvector/Profile 校验 → Chat/Embedding Probe → 分阶段启用 → 虚构向量/Backfill/Lease/范围 Probe → B3-A 词法回归与清理。
