...
1. 在 ProjectAI 点击一次“确认本次工时”，由统一校验确认任务、项目、正常工时、加班工时、内部分类和状态；低置信度只提示，不需要逐条审核。内部分类不会写入 WeCom。
...
6. 检查每项返回 `validated`/Dry Run 验证结果，同时企业微信任务列表没有新增任务；Dry Run 不把 ProjectAI 任务标记为 submitted。
7. 确认单条保存和最终提交均未被点击。
