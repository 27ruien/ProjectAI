...
npm run release:preflight -- --manifest=<manifest-json> --production-inventory=<sanitized-json> --checks=<checks-json>
...
npm run release:go-no-go -- --checklist=<sanitized-json>
```

Tracked 文件只保存 Schema、模板和虚构 fixture。Release Artifact 禁止包含 Secret、完整 Env、Cookie、Session Token、数据库密码、对象 Key、客户内容、完整 Prompt/问题、Query/Document Vector 或 Provider Payload。
...
`release:go-no-go` 对所有 required item fail-closed。只有 main/CI/Evidence/Image、Production 基线、Backup/Checksum、Restore、Migration、旧 App + 0007、新 App disabled、Rollback、磁盘、Nginx、Compose、Secret、成本、监控、Staging/Rehearsal 以及 P0/P1 全部通过才返回 `GO`。B3-C1 PR 必须保持 Draft；`GO` 也不授权 Production Rollout。
