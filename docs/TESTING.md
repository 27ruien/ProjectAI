...
5. Production build + SSR/路由：`npm test`（已经包含一次 `npm run build`）。
6. 浏览器身份、隔离和原 MVP 主流程：`npm run test:e2e`。
7. 完整 MVP 验证：`npm run qa:mvp`；CI 额外显式运行数据库集成测试。
8. Staging：数据库/应用健康、认证 Cookie、Session 刷新/退出、角色与跨项目边界、HTTP/MIME/Nginx 和 Production 不变回归。
...
2026-07-14 本地已从空测试库连续执行两次受保护 Reset/Migration/Seed，再执行第三次 insert-only Seed，并验证 Seed 对已有修改为非破坏性；当前 20/20 集成测试通过。该结果只证明已执行的本地集成层，不能替代 GitHub Actions 或 Staging 运行证据。
...
PR 与 main push 运行 Node 22、npm cache、Chromium、PostgreSQL 17、Migration、Seed、typecheck、lint、一次 production build + SSR、当前 20 条授权/审计/逐项目审核权限集成测试和 Playwright；新提交取消同分支旧任务，不进行部署。
...
- 本地已通过：`npm run typecheck`、`npm run lint`、PostgreSQL/权限集成测试 20/20、`npm test`（production build + SSR 4/4）和 Playwright 11/11；Action 持久化/水合并发压力 5/5 通过。
- 本地产品审查 artifacts 已生成并目检：6/6 必需截图、`manifest.json` 完整、客户端构建 Seed 业务内容扫描通过；真实附件的脱敏副本已完成本地结构/归档扫描验证，内嵌/改名/嵌套/SFX ZIP、gzip、路径名、超长 Session JSON、二进制结构化数据、折行 Base64、MIME/原始 percent-encoded Data URI、缺失证据及数据库不可验证失败关闭回归为 10/10。
- 待确认：Draft PR 的 GitHub Actions、CI artifact 实际上传内容，以及 v0.3 Staging 数据库/应用/公网身份边界。不得把本地结果替代这些外部运行证据。
