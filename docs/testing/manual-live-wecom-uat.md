...
3. 明确记录页面的 AI 模式。Mock 只验证流程；真实 AI 质量验收需显式设置
   `UAT_AI_PROVIDER=real` 并使用服务端 Secret，未配置时不得伪装或回退。
4. 对三条虚构随记执行 AI 整理，确认完成/进行中/待处理状态、工时和进度均有来源；
   低置信字段显示提示，但页面不存在逐条“标记已审核”。
5. 执行拆分、合并和修改；点击一次“确认本次工时”，确认只变为 confirmed、未同步、
   未清空活动任务。复制并下载服务端权威 JSON。
6. 修改已确认草稿，确认状态退回待确认；重新确认后才显示可用同步动作。
7. 使用 Local Mock SmartSheet 验证同日两批：首批 verified saved 后进入只读历史，第二批
   只包含新随记；failed 只重试失败项，unknown 先人工核对。
8. 使用 UAT Restricted User 登录，确认不能读取、修改、导出或同步另一用户/项目日报。
9. 运行 `npm run test:uat:flags`，确认日报 Flag 关闭时 UI/API 均拒绝，WeCom Flag
   关闭时同步 UI/API 均拒绝，Real AI 未配置时 AI 入口禁用。

## 真实 AI 质量验收

Mock 自动化不能替代本节。Real 模式至少输入“完成企业微信日报同步方案整理，耗时 1 小时，已完成”“检查扩展字段映射，约 30 分钟，目前仍在进行中”“准备验证腾讯文档写入流程，尚未开始”。人工核对三项合理拆分、描述来自输入、明确/近似/缺失工时处理、三种状态、无虚构项目，并确认不出现固定的“虚构项目工作记录 1/2/3”。未实际运行时结果必须写 **NOT RUN**。
...
这不影响 Mock AI、Mock SmartSheet 和 Local UAT 的各自自动化结果，但这些结果均不能替代真实 AI 质量或真实连接器验收；在该阻塞关闭前不能宣称真实 WeCom 可用。
